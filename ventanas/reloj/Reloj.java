    ...
    public class Reloj extends JFrame implements Runnable {
        ...
        private Clip ticTac,doceNoche,horaNueva;
        ...
        public void cargarSonidos(String tictac, String midNight, String newHour) {
            try {
        ...
        public void hacerTicTac() {
            ...
            if (doceNoche != null && cal.get(Calendar.HOUR) == 0 && cal.get(Calendar.MINUTE) == 0) {
                doceNoche.start();
            } else {
                doceNoche.stop();
                doceNoche.setFramePosition(0);
            }
            if (doceNoche != null && cal.get(Calendar.HOUR) == 6 && amPm == cal.get(Calendar.PM)) {
                doceNoche.start();
            } else {
                doceNoche.stop();
                doceNoche.setFramePosition(0);
        ...
        public void dibujarIndicadores(Graphics g, int centroX, int centroY, int radio) {
            ...
            for (byte i = 0; i < 12; i++) {
                ...
                if(i == 6) g.drawString("VI", centroX, centroY + 205);
                if(i == 7) g.drawString("VII", centroX - 105, centroY + 175);
                if(i == 8) g.drawString("VIII", centroX - 175, centroY + 100);
                if(i == 9) g.drawString("IX", centroX - 205, centroY);
                if(i == 10) g.drawString("X", centroX - 175, centroY - 100);
                if(i == 11) g.drawString("XI", centroX - 105, centroY - 175);
            }
